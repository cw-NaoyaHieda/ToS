...
# 重点サンプリングの関数の関数も新しいものにした。
  ...
  #par2 f # par g
    dfas2(x, mu=par2[1], sigma=par2[2],
                       lambda=par2[3], delta=par2[4])/
    dfas2(x, mu=par[1], sigma=par[2],
                       lambda=par2[3], delta=par2[4])}
  
  #weightを計算する
  w1 <- c(f(rfa1, par = fit$par2, par2=c(mu.val5, fit$par2[2:4])))
  w25 <- f(rfa25, par = fit$par2, par2=c(mu.val5, fit$par2[2:4]))
  ...
  out1<- parSapply(cl, 100:N, function(x){
    ...
    if(is.null(dim(A[which.min(abs(A[,3]-0.01)),]))){
  ...
  out25<-parSapply(cl, 100:N, function(x){
    ...
    if(is.null(dim(A[which.min(abs(A[,3]-0.025)),]))){
  ...
  out5<-parSapply(cl, 100:N, function(x){
    ...
    if(is.null(dim(A[which.min(abs(A[,3]-0.05)),]))){
