...
適当に`mle_w <- seq(0.01, 1, 0.99/249)`を対数尤度にかけて最適化してみました。

```{r rolling_wegiht,eval=FALSE}
IS.fa.outs <- c()
IS.norm.outs <- c()
SMC.fa.outs <- c()
SMC.norm.outs <- c()
...
for(i in 2:(length(df$log_x)-249)){
  ...
  mle_w <- seq(0.01, 1, 0.99/249)
  fit <- mle.dfas2_weight(x = rt, weight =  mle_w, ini = first_parameter)
  clusterExport(cl,list("fit"))
  rand.fa<-rfa_SIR_para(n=10000, mu=fit$par2[1],
                        sigma = fit$par2[2],
                        lambda = fit$par2[3],
  ...
  #SMC.norm.outs <- rbind(SMC.norm.outs,SMC.norm.out)
  print(i)
  result3 <- rbind(result3,
                   c(i, IS.fa.out, fit$par2))
  old_parameter <- fit$par2
}

stopCluster(cl)
save(list=c("result3"),file="data/rolling_result_useoldpara_weight.Rdata")
```

```{r}
load("data/rolling_result_useoldpara_weight.Rdata")
colnames(result3) <- c("dt",
                      "IS_VaR_fa_0.01","IS_ES_fa_0.01",
                      "IS_VaR_fa_0.025","IS_ES_fa_0.025",
                      "IS_VaR_fa_0.05","IS_ES_fa_0.05",
                      "mu","sigma","lambda","delta")

result3_VaR <- result3[,c(1,2,4,6)] %>% data.frame()

result3_ES <- result3[,c(1,3,5,7)]  %>% data.frame()

result3_para <- result3[,c(1, 8, 9, 10, 11)]  %>% data.frame()
```

```{r }
result3_VaR$dt <- df$dt[c(251:length(df$dt))]
result3_ES$dt <- df$dt[c(251:length(df$dt))]
result3_para$dt <- df$dt[c(251:length(df$dt))]
plot3_VaR <- result3_VaR %>% data.frame() %>% melt(id="dt")
plot3_ES <- result3_ES %>% data.frame() %>% melt(id="dt")
plot3_para <- result3_para %>% data.frame() %>% melt(id="dt")

plot3_VaR$level <- apply(plot3_VaR,1,function(x) substr(x["variable"],
                                    regexpr("0.0", x["variable"]),
                                    nchar(x["variable"])))

plot3_VaR$method <- apply(plot3_VaR,1,function(x)
  paste(strsplit(x["variable"][1] %>% as.character(),"_")[[1]][1],
             strsplit(x["variable"] %>% as.character(),"_")[[1]][3]," weight")
  )


plot3_ES$level <- apply(plot3_ES,1,function(x) substr(x["variable"],
                                    regexpr("0.0", x["variable"]),
                                    nchar(x["variable"])))

plot3_ES$method <- apply(plot3_ES,1,function(x)
  paste(strsplit(x["variable"][1] %>% as.character(),"_")[[1]][1],
             strsplit(x["variable"] %>% as.character(),"_")[[1]][3]," weight")
  )

ggplot() +
  geom_line(data=plot3_VaR,
    aes(x=dt,y=value,colour=method)) +
  geom_line(data=plot_VaR,
    aes(x=dt,y=value,colour=method)) +
  geom_line(data=df[c(252:length(df$dt)),],
...
ggplot() +
  geom_line(data = plot3_ES,aes(x=dt,y=-value,colour=method))+
...
```{r}
#VaR
check3_VaR <- apply(data.frame(df$log_x[251:length(df$log_x)], -result3_VaR[,-1]), 1,
      function(x) x[1] > x[-1]) %>% t()

count3_VaR <- colSums(check3_VaR)
print("Check count over Estimate VaR")
print("weight var")
count3_VaR/ dim(check3_VaR)[1]

#ES?
eval3_ES <- check3_VaR * apply(result3_ES[,-1],2,function(x) abs(df$log_x[c(251:length(df$dt))] - x)/ -x)
print("normal var")
colMeans(eval3_ES)
```
...
## lambda=0なら初期値を0にして最適化


```{r rolling_wegiht2,eval=FALSE}
IS.fa.outs <- c()
IS.norm.outs <- c()
SMC.fa.outs <- c()
SMC.norm.outs <- c()
...
stopCluster(cl)
save(list=c("result3"),file="data/rolling_result_useoldpara_weight_lambdashift.Rdata")
```

```{r}
load("data/rolling_result_useoldpara_weight_lambdashift.Rdata")
colnames(result3) <- c("dt",
                      "IS_VaR_fa_0.01","IS_ES_fa_0.01",
                      "IS_VaR_fa_0.025","IS_ES_fa_0.025",
                      "IS_VaR_fa_0.05","IS_ES_fa_0.05",
...
```{r}
ggplot(data=data.frame(x=c(-6,-3)),aes(x=x))+
...
```{r}
ggplot(data=data.frame(x=c(-10,-2.5)),aes(x=x))+
  stat_function(fun = f,
                aes(color="0.01"),
...
result2<-foreach(i = 2:(length(df$log_x)-249), .combine='cbind') %dopar% {
  ...
  para <- c(result[ , i-1, c("sigma","lambda","delta")])
  ...
  # 99%,97.5%,95%それぞれのVaRと平均が一致するthetaを取得
  rfa.IS.1<-rIS_SIR2(n=20000, par=c(VaR.true.FA[1], para[2:4]),
                     par2=c(VaR.true.FA[1], sd(rt)))
  rfa.IS.25<-rIS_SIR2(n=20000, par=c(VaR.true.FA[2], para[2:4]), 
                      par2=c(VaR.true.FA[2], sd(rt)))
  rfa.IS.5<-rIS_SIR2(n=20000, par=c(VaR.true.FA[3], para[2:4]), 
                     par2=c(VaR.true.FA[3], sd(rt)))
  # サンプリングしたものを入力としてFA分布の重点サンプリングを行う
  rfa1 <- sample(rfa.IS.1$q, 10000)
  rfa25 <- sample(rfa.IS.25$q, 10000)
  rfa5 <- sample(rfa.IS.5$q, 10000)
...
# 重み付きパラメータ推定 + ロケーションシフト

重み付きで推定したパラメータを用いて、location shift
...
clusterExport(cl,list("s_inverse","dfas","dfas2","Cf","Sf",
                      "cl_l","rfa_SIR","Resample1"))
result4 <- c()
result4<-foreach(i = 2:(length(df$log_x)-249), .combine='cbind') %dopar% {
  rt <- df$log_x[i:(i+249)]
  rt <- rt[rt!=0]
  #パラメータ推定の結果は、先ほどのものを使い回す。
  para <- c(result3[i-1, c("mu","sigma","lambda","delta")])
...
stopCluster(cl)
save(list=c("result4"),file="data/rolling_result_locationshift_weight.Rdata")
```


```{r}
load("data/rolling_result_locationshift_weight.Rdata")
result4 <- t(result4)
...
```{r}
result4_VaR$dt <- df$dt[c(251:length(df$dt))]
result4_ES$dt <- df$dt[c(251:length(df$dt))]
result4_para$dt <- df$dt[c(251:length(df$dt))]
plot4_VaR <- result4_VaR %>% data.frame() %>% melt(id="dt")
plot4_ES <- result4_ES %>% data.frame() %>% melt(id="dt")
plot4_para <- result4_para %>% data.frame() %>% melt(id="dt")

plot4_VaR$level <- apply(plot4_VaR,1,function(x) substr(x["variable"],
                                    regexpr("0.0", x["variable"]),
                                    nchar(x["variable"])))

plot4_VaR$method <- apply(plot4_VaR,1,function(x)
  paste(strsplit(x["variable"][1] %>% as.character(),"_")[[1]][1],
             strsplit(x["variable"] %>% as.character(),"_")[[1]][3]," location shift + weight")
  )


plot4_ES$level <- apply(plot4_ES,1,function(x) substr(x["variable"],
                                    regexpr("0.0", x["variable"]),
                                    nchar(x["variable"])))

plot4_ES$method <- apply(plot4_ES,1,function(x)
  paste(strsplit(x["variable"][1] %>% as.character(),"_")[[1]][1],
             strsplit(x["variable"] %>% as.character(),"_")[[1]][3]," location shift + weight")
  )
...
ggplot() +
  geom_line(data = plot_ES %>% filter(method == "IS fa"),aes(x=dt,y=-value,colour=method))+
  geom_line(data = plot4_ES,aes(x=dt,y=-value,colour=method))+
  geom_line(data = plot2_ES,aes(x=dt,y=-value,colour=method))+
  geom_line(data=plot3_ES,
    aes(x=dt,y=-value,colour=method)) +
  theme_bw()+
  facet_grid(level~.,scales = "free")+
  ggtitle("ES")
...
```{r}
#VaR
check4_VaR <- apply(data.frame(df$log_x[251:length(df$log_x)], -result4_VaR[,-1]), 1,
      function(x) x[1] > x[-1]) %>% t()

count4_VaR <- colSums(check4_VaR)
print("Check count over Estimate VaR")
print("weight location shift var")
count4_VaR/ dim(check3_VaR)[1]

#ES?
eval4_ES <- check3_VaR * apply(result4_ES[,-1],2,function(x) abs(df$log_x[c(251:length(df$dt))] - x)/ -x)
print("normal var")
colMeans(eval4_ES)
```