...
result<-foreach(i = 2:(length(df$log_x)-249), .combine='cbind') %dopar% {
  ...
  c(i,IS.fa.out,IS.norm.out,SMC.fa.out,SMC.norm.out,fit$par2,mean(rt), sd(rt))
}

stopCluster(cl)
save(list=c("result"),file="data/rolling_result.Rdata")
```


```{r}
load("data/rolling_result.Rdata")
result <- t(result)
